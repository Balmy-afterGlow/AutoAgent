...
def _process_and_queue(
    ...
    try:
        ...
        print(
            f"Error processing instance {instance.get('instance_id', 'unknown')}: {str(e)}"
        )
        traceback.print_exc()
...
def run_evaluation(
    ...
    if metadata is not None:
        logger.info(f"Evaluation started with Agent {metadata.agent_func}\n")
    else:
        logger.info(
            "Running evaluation without metadata.", title="Warning", color="red"
        )
    ...
    try:
        if use_multiprocessing:
            ...
            while instances_completed < total_instances:
                ...
                while (
                    len(active_processes) < num_workers
                    and instances_completed < total_instances
                ):
                    try:
                ...
                for p, start_time in active_processes[:]:
                    if not p.is_alive():
                        try:
                            ...
                            if p.is_alive():
                                logger.warning(
                                    f"Process {p.pid} cleanup timeout, force terminating..."
                                )
                                p.terminate()
                                p.join(timeout=5)
                                if p.is_alive():
                            ...
                            logger.warning(
                                f"Error cleaning up process {p.pid}: {str(e)}"
                            )
                            p.kill()
                        finally:
...
def _process_instance_wrapper(
    ...
    if use_mp:
        log_path = os.path.join(
            ...
            "logs",
            f'agent_{metadata.model}_did_{instance["instance_id"]}.log',
        )
