...
terminal_env = RequestsMarkdownBrowser(
    local_root=os.getcwd(), workplace_name="terminal_env", viewport_size=1024 * 8
)
...
def _get_browser_state(env: RequestsMarkdownBrowser) -> Tuple[str, str]:
    ...
    header += (
        f"Terminal viewport position: Showing page {current_page+1} of {total_pages}.\n"
    )
    if total_pages > 1:
    ...
    try:
        ...
        if Path(code_path).is_absolute():
            if env.run_command(f"ls {code_path}")["status"] != 0:
                return f"[ERROR] File {code_path} does not exist"
            code_abs_path = code_path
        else:
            code_abs_path = f"{env.docker_workplace}/{code_path}"
            if env.run_command(f"ls {code_abs_path}")["status"] != 0:
                return f"[ERROR] You use a relative path, so we regard the `{env.docker_workplace}` as the root of the workplace, but `{code_abs_path}` does not exist"
        ...
        if cwd:
            ...
            if Path(cwd).is_absolute():
                ...
                if env.run_command(f"ls {cwd}")["status"] != 0:
                    return f"[ERROR] You use a relative path for `cwd`, so we regard the `{env.docker_workplace}` as the working directory, but `{cwd}` does not exist"
        else:
