...
def get_coding_agent(model: str, **kwargs) -> Agent:
  ...
  When you think you have completed the task the `System Triage Agent` asked you to do, you should use `transfer_back_to_triage_agent` to transfer the conversation back to the `System Triage Agent`.

  When you need additional information (such as an API key) or perform sensitive operations, you need to use `transfer_back_to_triage_agent` to transfer the proxy to the `System Triage Agent`, use the `case_not_resolved` tool to exit the loop, and issue a new query to the user or request user confirmation.

  You should do your best to complete the task until you need some additional information or it is really impossible to solve it, then use `transfer_back_to_triage_agent` to return to the `System Triage Agent` and let it make the next decision.
