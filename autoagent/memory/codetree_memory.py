...
class CodeTreeMemory(Memory):
    ...
    def add_code_files(
        self, directory: str, exclude_prefix: List[str] = ["workplace_"]
    ):
        """
        将指定目录下的所有代码文件添加到内存中。
        ...
        parsed_snippets = parser.parse_directory(directory)
        snippet_texts = list(
            map(lambda x: x.snippet.decode("ISO-8859-1"), parsed_snippets)
        )
        embedded_texts = self.embedder.embeddings.create(
            input=snippet_texts, model="text-embedding-3-small"
        ).data
        embedded_snippets = []
    ...
    def query_code(self, query_text: str, n_results: int = 5) -> List[Dict]:
        ...
        results = self.client.get_or_create_collection(self.collection_name).query(
            query_embeddings=[query_embedding], n_results=n_results
        )
        return [
            {"file": metadata["filenames"], "content": doc}
...
if __name__ == "__main__":
    code_memory = CodeTreeMemory(
        ...
        db_name="code_tree",
        platform="OpenAI",
        api_key="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",
    )

    # 添加代码文件到内存
    code_memory.add_code_files(
        "/Users/tangjiabin/Documents/reasoning/SelfAgent/workplace_test/SelfAgent",
