...
def _checkMeetChallenge():
    ...
    while check_count <= max_attempts:
        ...
        if (
            page.get_by_text("请完成以下操作，验证您是真人。").count() == 0
        ...
        print(
            f"检测到 Cloudflare 验证页面，尝试处理... (第 {check_count}/{max_attempts} 次)"
        )
...
class BrowserEnv:
    ...
    def browser_process(self):
        if self.eval_mode:
            ...
            debug_print(
                True,
                "Initializing browser env for web browsing evaluation.",
                title="Browser Env",
                log_path=self.log_path,
            )
            ...
            normalized_source = normalized_source.replace(
                "docker_workplace = None",
                f"docker_workplace = {repr(self.docker_workplace)}",
            )

            action_set = HighLevelActionSet(
                subsets=["chat", "infeas", "bid", "nav", "tab", "custom"],
                custom_actions=[
                    _visit_page,
                    _click_id,
                    _get_page_markdown,
...
"""
            ...
            try:
                if self.browser_side.poll(timeout=0.01):
                    ...
                    if unique_request_id == "SHUTDOWN":
                        debug_print(
                            False,
                            "SHUTDOWN recv, shutting down browser env...",
                            title="Browser Env",
                            log_path=self.log_path,
                        )
                ...
                debug_print(
                    True,
                    "Browser env process interrupted by user.",
                    title="Browser Env",
                    log_path=self.log_path,
                )
    ...
    def step(self, action_str: str, timeout: float = 30) -> dict:
        ...
        while True:
            if should_exit() or (
                time.time() - start_time > timeout and "_visit_page" not in action_str
            ):
                raise TimeoutError("Browser environment took too long to respond.")
            if should_exit() or (
                time.time() - start_time > 600 and "_visit_page" in action_str
            ):
                raise TimeoutError("Browser environment took too long to respond.")
    ...
    def check_alive(self, timeout: float = 60):
        ...
        if self.agent_side.poll(timeout=timeout):
            ...
            debug_print(
                True,
                f"Browser env is not alive. Response ID: {response_id}",
                title="Browser Env",
                log_path=self.log_path,
            )
    ...
    def close(self):
        ...
        try:
            ...
            debug_print(
                True,
                "Encountered an error when closing browser env",
                exc_info=True,
                title="Browser Env",
                log_path=self.log_path,
            )
