...
def delete_agent(agent_name: str, context_variables):
    ...
    try:
        ...
        return (
            "[ERROR] Before deleting a agent, you should list all agents first. But the following error occurred: "
            + str(e)
        )
    ...
    try:
        ...
        if result["status"] != 0:
            return (
                f"[ERROR] Failed to delete agent: `{agent_name}`. Error: "
                + result["result"]
            )
...
def run_agent(
    ...
    if model not in ["claude-3-5-sonnet-20241022"]:
        return (
            "[ERROR] The model "
            + model
            + " is not supported. Supported models: claude-3-5-sonnet-20241022."
        )
    env: Union[LocalEnv, DockerEnv] = context_variables.get("code_env", LocalEnv())
    try:
        path = get_metachain_path(env)
    ...
    if isinstance(ctx_vars, dict) is False:
        try:
            ...
            return (
                "[ERROR] The context variables are not a valid JSON object. Error: "
                + str(e)
            )
...
"""
        ...
        create_file(
            f"{path}/tmp_shell/run_agent_{timestamp}.sh",
            shell_content,
            context_variables,
        )
...
"""
    ...
    try:
        msg = create_file(
            agents_dir + "/" + agent_name.lower().replace(" ", "_") + ".py",
            create_codes,
            context_variables,
        )
        ...
        result = code_env.run_command(
            "cd {} && python autoagent/agents/{}.py".format(
                path, agent_name.lower().replace(" ", "_")
            )
        )
        if result["status"] != 0:
