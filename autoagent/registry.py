...
def truncate_output(output: str, max_length: int = MAX_OUTPUT_LENGTH) -> str:
    ...
    if len(tokens) > max_length:
        return (
            decode_tokens_by_tiktoken(tokens[:max_length])
    ...
    def register(
        ...
        def decorator(func: Callable):
            ...
            while body_lines and (
                body_lines[0].strip().startswith("@") or "def " in body_lines[0]
            ):
            ...
            self._registry[registry_type][func_name] = (
                wrapped_func if type == "plugin_tool" else func
            )
