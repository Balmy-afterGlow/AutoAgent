    ...
    def init_container(self):
        ...
        if self.git_clone:
            if not os.path.exists(os.path.join(self.local_workplace, "AutoAgent")):
                ...
                git_command = [
                    ...
                    "-b",
                    self.test_pull_name,
                    f"https://{AI_USER}:{GITHUB_AI_TOKEN}@github.com/HKUDS/AutoAgent.git",
                ]
                print(git_command)
                ...
                if result.returncode != 0:
                    raise Exception(
                        f"Failed to copy .env file to the AutoAgent directory. Error: {result.stderr}"
                    )
                # create a new branch
        ...
        if existing_container.stdout.strip() == self.container_name:
            ...
            running_container = subprocess.run(
                running_check_command, capture_output=True, text=True
            )
        ...
        if func.__doc__:
            try:
                if "{docker_workplace}" in func.__doc__:
                    wrapped.__doc__ = func.__doc__.format(
                        docker_workplace=env.docker_workplace
                    )
                else:
                    wrapped.__doc__ = func.__doc__
                if "{local_workplace}" in func.__doc__:
                    wrapped.__doc__ = func.__doc__.format(
                        local_workplace=env.local_workplace
                    )
                else:
                    wrapped.__doc__ = func.__doc__
