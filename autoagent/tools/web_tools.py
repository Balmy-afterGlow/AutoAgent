...
@register_tool("click")
def click(
    context_variables, bid: str, button: Literal["left", "middle", "right"] = "left"
):
    """
    Clicks the mouse on the target with the given element bid.
    Args:
...
def truncate_by_tokens(
    ...
    create_directory(
        f"{env.docker_workplace}/web_page_markdown_output",
        context_variables={"code_env": env},
    )
    output_path = (
        f"{env.docker_workplace}/web_page_markdown_output/full_output_{timestamp}.md"
    )
    create_msg = create_file(
        output_path, content=text, context_variables={"code_env": env}
    )
    # 截断tokens并解码回字符串
...
if __name__ == "__main__":
    env = BrowserEnv(
        ...
        local_root="/Users/tangjiabin/Documents/reasoning/autoagent",
