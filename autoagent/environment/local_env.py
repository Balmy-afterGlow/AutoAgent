...
class LocalEnv:
    def __init__(self, docker_config: DockerConfig = None):
        if docker_config is None:
            ...
            self.local_workplace = osp.join(
                docker_config.local_root, docker_config.workplace_name
            )
            self.docker_workplace = osp.join(
                docker_config.local_root, docker_config.workplace_name
            )
    ...
    def run_command(self, command, stream_callback=None):
        ...
        # 发送最终的完整响应
        response = {"status": process.poll(), "result": output}
        return response
