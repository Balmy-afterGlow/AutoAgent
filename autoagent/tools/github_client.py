...
class GitHubClient:
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token:
            raise ValueError(
                "GitHub Token is required, please provide it via the token parameter or set the GITHUB_AI_TOKEN environment variable."
            )
...
class GitHubSearcher:
    ...
    def get_contents_batch(self, items: List[Dict]) -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_item = {
                executor.submit(self._get_single_content, item): item for item in items
            }
